...
" This file is the common configuration file for both Neovim (NVIM) and Vim.
...
set nobackup
set title     " Update terminal title once Vim started.
set titleold= " Restore previous terminal title after leaving Vim.
...
" <Leader>at == Align Table
map <Leader>at :Tabularize /\t/<CR><CR>:retab<CR>
" <Leader>ab == Align ta[B]s
map <Leader>ab :Tabularize /\t/<CR><CR>
...
"         mz           Mark position with 'z', so the cursor will return to it.
"         `z           Jump back to mark 'z', restoring the cursor where it was before pressing F5.
"         echoerr ""   Reset error message on the command line to "".
...
" HDCalc - Calculator - Calculates expression on the current line.
" ══════════════════════════════════════════════════════════════════════════════
nnoremap <Leader>= :call HDCalcLineExpr()<CR>

function! HDCalcLineExpr()
  " Remove any '=' sign preceeded by spaces, if any are there.
  let l:expr = substitute(getline('.'), '^ *=', '', '')
  try
    let l:result = eval(l:expr)
    call append(line('.'), l:result)
  catch
    echohl ErrorMsg
    echom "Error evaluating expression: " . v:exception
    echohl None
  endtry
endfunction

" Define 'HDCalc' which then can be called in command-line mode as ':HDCalc'.
command! -nargs=0 HDCalc call HDCalcLineExpr()
